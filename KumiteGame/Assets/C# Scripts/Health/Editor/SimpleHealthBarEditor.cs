...
public class SimpleHealthBarEditor : Editor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		if( EditorGUILayout.BeginFadeGroup( StatusImageAssigned.faded ) )
		{
			...
			if( targ.colorMode == HealthBar.ColorMode.Single )
			...
			{
				serializedObject.ApplyModifiedProperties();
				DisplayTextOptions.target = targ.displayText != HealthBar.DisplayText.Disabled;
			...
			if( EditorGUILayout.BeginFadeGroup( DisplayTextOptions.faded ) )
			{
				...
				{
					case HealthBar.DisplayText.Percentage:
					{
						EditorGUILayout.LabelField( "Text Preview: " + targ.additionalText + testValue + "%" );
					}
					break;
					case HealthBar.DisplayText.CurrentValue:
					{
						EditorGUILayout.LabelField( "Text Preview: " + targ.additionalText + testValue );
					}
					break;
					case HealthBar.DisplayText.CurrentAndMaxValues:
					{
	...
	void StoreReferences ()
	{
		...
		DisplayTextOptions = new AnimBool( targ.displayText != HealthBar.DisplayText.Disabled );
		...
		exampleCode = "HealthBar.UpdateBar( \"" + targ.barName + "\", currentValue, maxValue );";
	...
	void UpdateColor ()
	{
		...
		{
			case HealthBar.ColorMode.Single:
			{
				targ.barImage.color = targ.barColor;
			} break;
			case HealthBar.ColorMode.Gradient:
			{
	...
	bool GetColorWarning ()
	{
		...
		if( targ.colorMode == HealthBar.ColorMode.Single && targ.barImage.color != targ.barColor )
			return true;
	...
	void GenerateExampleCode ()
	{
		switch( functionList )
		{
			...
			{
				exampleCode = "HealthBar.UpdateStatusColor( \"" + targ.barName + "\", newColor );";
			}
			break;
			case FunctionList.UpdateTextColor:
			{
				exampleCode = "HealthBar.UpdateTextColor( \"" + targ.barName + "\", newTextColor );";
			}
			break;
		}
	}
